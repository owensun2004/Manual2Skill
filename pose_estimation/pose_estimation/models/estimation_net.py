...
# from .backbone.lora_vit import LoRA_SimpleViT
from utils import *
import torch
...
class PoseEstimationNetwork(nn.Module):
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.image_encoder = DeepLab_V3_plus(num_classes = 1, input_channel=3, out_dim=self.cfg.network.hidden_size * 2)
        self.pts_encoder = PointNet2(in_dim=4, hidden_dim=128, out_dim=self.cfg.network.hidden_size)
        self.pose_regressor = mlp(input_dim=self.cfg.network.hidden_size*(4 + 2 + 1))
    ...
    def init_optimizer_scheduler(self):
        ...
        self.optimizer = OptimizerManager([optimizer1, optimizer2, optimizer3])
        self.scheduler = SchedulerManager([scheduler1, scheduler2, scheduler3])
    ...
    def forward(self, batch_data):
        ...
        #overfitting set
        # pts_w_mask = torch.cat([pts, mask.unsqueeze(-1)], dim=-1)
        ...
        pts_feat_global = mean_max_pool(pts_feat_padding, valid_mask)
        # print(img_feat.shape, pts_feat_padding.shape, pts_feat_global.shape)
        mlp_input_tensor = torch.cat([img_feat.unsqueeze(1).repeat(1,P,1), pts_feat_padding,
                                        pts_feat_global.unsqueeze(1).repeat(1,P,1)], dim=-1)[valid_mask]
